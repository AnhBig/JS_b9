...
function creatEmploye() {
    ...
    for(var i = 0; i < employeList.length; i++) {
        if(employeList[i].employTk === employeTk) {//=> mảng chứa đối tượng nv của mk
    ...
    var employe = new Employ(employeTk,
        employeName,
        employeEmail,
        employePass,
        employeDatePicker,
...
function renderEmploye() {
    var html = "";
for(var i = 0; i < employeList.length; i++) {//=> data truyền vô mảng nào thì in mảng đó chứ ko phải lúc nào cx in theo ds nv, ta sẽ cho truyền vào 1 datta và truyền mảng nào thì in mảng đó
    html +=`
    <tr>
    ...
    <td>
    <button onclick = "deleteEmploy( '${employeList[i].studentId}' )" class = "btn btn-danger">Xóa</button>
    </td> 
    </tr>`
}
//   console.log(html);
...
//DELETEDEMPLOYE:
function deleteEmploy(employTk) {//gắn cho cái button xóa ở bên html

    var index = findByTk(employTk); //truyền vô cái TK mà mk đang muốn xóa, tìm xong sẽ return về index
...
function findByTk(tk) {
    for(var i = 0; i < employeList.length; i++) {
     if(employeList[i].employTk === tk) { //dò từ đầu đến cuối ds xem có ai có id giống vsid mk cần tìm
